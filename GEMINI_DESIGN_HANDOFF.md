...
고정 지출은 일반 소비 예산과 별도 관리합니다.

- 고정 지출 총 예산: `1,450,000원`
- 화면에서는 체크 완료 금액, 남은 고정 예산, 고정 지출 예산 사용률을 표시합니다.
- 이 예산은 현재 `src/app/page.tsx`의 `fixedExpenseBudget` 상수로 관리됩니다.
- 고정 지출 체크는 Notion 소비 내역에 자동 저장되지 않습니다.
...
현재 앱은 스크롤 앵커가 아니라 상태 기반 페이지 전환으로 나뉩니다.

- `대시보드`: 이번 주기 총 지출, 인사이트, 카테고리별 게이지, 예산 설정
- `지출`: 빠른 지출 입력 폼, 최근 소비 내역, 소비 삭제
- `고정지출`: 고정 지출 추가, 체크, 삭제, `1,450,000원` 고정 예산 사용률

주의: 하단 내비게이션은 섹션으로 스크롤시키는 용도가 아니라 위 3개 페이지를 전환하는 용도입니다. 디자인을 바꿀 때도 `지출`과 `고정지출`을 한 화면에 다시 합치지 않는 것이 좋습니다.
...
2. 체크박스는 현재 17일 기준 주기의 납부 여부만 토글
3. 체크된 고정 지출 금액 합계로 `1,450,000원` 예산 대비 사용률과 남은 금액을 계산
4. 고정 지출은 실제 Notion 소비 내역에 자동 반영되지 않음
...
- 상단 탭과 하단 내비게이션은 `대시보드`, `지출`, `고정지출` 3페이지 흐름을 유지합니다.
- 고정 지출은 최근 소비 내역과 분리된 별도 페이지처럼 보여야 합니다.
- 하단 내비게이션은 모바일 조작성을 위한 핵심 요소입니다.
